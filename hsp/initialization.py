...
def check_parameters(d, check_exist=True):
    """
    Check that the provided dictionary defines all mandatory hsp arguments.

    Args:
        d: python dictionary
    """
    # verify that input files paths are defined
    if 'data' not in d or len(d['data']) < 1:
    ...
    for img in d['data']:
        if not dict_has_keys(img, ['img']):
            ...
            sys.exit(1)
        if check_exist and not os.path.exists(img['img']):
            print('ERROR: invalid img paths for image', img)
            sys.exit(1)

    # verify that tasks are defined
    if 'tasklist' not in d or len(d['tasklist']) < 1:
        print('ERROR: missing task list')
        sys.exit(1)
    for task in d['tasklist']:
        if 'type' not in task and 'template' not in task:
            print('ERROR: missing type/template for task', task)
            sys.exit(1)
        if 'type' in task:
            filepath = common.tasktemplate(task['type'])
            if not filepath:
                print('ERROR: CANNOT find task template in {HSP_DATA} for task', task)
                sys.exit(1)
            ...
            if 'type' not in task:
                task['type'] = 'uno'
            if check_exist:
                if not os.path.exists(task['template']):
                    print('ERROR: invalid template file for task', task)
                    sys.exit(1)
                task['type'] = common.tasktype(task['template'])

    # warn about unknown parameters. The known parameters are those defined in
    # the global config.cfg dictionary, plus the mandatory 'data' and 'tasklist'
    for k in d.keys():
        if k not in ['data', 'tasklist']:
            if k not in cfg:
...
def build_cfg(user_cfg):
    """
    Populate a dictionary containing the hsp parameters from a user config file.
    ...
    cfg['tmpdir'] = os.path.expandvars(cfg['tmpdir'])
    if not os.path.isabs(cfg['tmpdir']):
        cfg['tmpdir'] = os.path.join(cfg['dstdir'], cfg['tmpdir'])

    # make sure that input data have absolute paths
    for i in range(len(cfg['data'])):
        for d in ['img']:
            if d in cfg['data'][i] and cfg['data'][i][d] is not None and not os.path.isabs(cfg['data'][i][d]):
                cfg['data'][i][d] = os.path.abspath(cfg['data'][i][d])
    for i in range(len(cfg['tasklist'])):
        if cfg['tasklist'][i]['template'] is not None and not os.path.isabs(cfg['tasklist'][i]['template']):
            cfg['tasklist'][i]['template'] = os.path.abspath(cfg['tasklist'][i]['template'])

def make_dirs():
    """
    Create directories needed to run hsp.
    """
    os.makedirs(cfg['dstdir'], exist_ok=True)
    os.makedirs(cfg['tmpdir'], exist_ok=True)
    print("output directory: {}".format(cfg['dstdir']))
    print("temporary directory: {}".format(cfg['tmpdir']))

    # store a json dump of the config.cfg dictionary
    with open(os.path.join(cfg['dstdir'], 'config.json'), 'w') as f:
