...
data_dir = os.path.join(parent_dir, 'template')
os.environ['PATH'] = bin_dir + os.pathsep + os.environ['PATH']

# global variable
# list of intermediary files generated by hsp
garbage = list()
...
def tasktemplate(task):
    """
    fetch task template file path
    """
    dirs = os.environ.get('HSP_DATA', data_dir).split(os.pathsep)
    for dirpath in dirs:
        filepath = os.path.join(dirpath,task)+'.xml'
        if os.path.exists(filepath):
            return filepath
    return None

def tasktype(xml, last=True):
    tree = et.parse(xml)
    if not tree:
        return "uno"
    ...
    if len(tasks) < 1:
        return "uno"
    if len(tasks) > 1 and not last:
        return "combo"
    return tasks[-1].attrib['name']
...
def tmpfile(ext=''):
    ...
    """
    fd, out = tempfile.mkstemp(suffix=ext, prefix='hsp_',
                               dir=os.path.expandvars(cfg['tmpdir']))
...
def meanstd_to_axb(mean, std):
    masked = (std!=0)
    c = np.zeros_like(std)
    c[masked] = 1/std[masked]
    return c, -mean*c

def compute_hstripe_meanstd(img, tile):
    rows, cols = img.shape
    n = rows//tile
    mean = np.zeros((n,cols), dtype=np.float32)
    std = np.zeros((n,cols), dtype=np.float32)
    for i in range(n):
        st = i*tile
        ed = (i+1)*tile if i<n-1 else rows-1
        d = img[st:ed, :]
        mean[i, :] = np.mean(d, axis=0)
        std[ i, :] = np.std(d, axis=0)
