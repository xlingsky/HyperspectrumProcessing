...
import hsp.utils.common as common

def show_progress(a):
    """
    Print the number of subtask that have been processed.
    ...
    status = "done {:{fill}{width}} / {} tasks".format(show_progress.counter,
                                                       show_progress.total,
                                                       fill='',
...
def tilewise_wrapper(fun, *args, **kwargs):
    """
    """
    if not kwargs['debug']:  # redirect stdout and stderr to log file
    ...
    if not kwargs['debug']:  # close logs
...
def launch_calls(fun, list_of_args, nb_workers, *extra_args, batchwise=False,
    ...
    Args:
        ...
        batchwise (bool): whether the calls are run batchwise or not
    ...
    for x in list_of_args:
        ...
        if batchwise:
            if type(x) == tuple:  # we expect x = (tile_dictionary, pair_id)
                log = os.path.join(x[0]['dir'], 'batch_%d' % x[1], 'stdout.log')
