...
/** double image data type
    ...
    with x and y integer.
 */
typedef struct image_datatype_s
{
  DataType * data;
  unsigned int xsize,ysize;
} * image_datatype;
 ...
 */
static void free_image_datatype(image_datatype i)
{
  if( i == NULL || i->data == NULL )
    error("free_image_datatype: invalid input image.");
  free( (void *) i->data );
  free( (void *) i );
}

/*----------------------------------------------------------------------------*/
/** Create a new image_datatype of size 'xsize' times 'ysize'.
 */
static image_datatype new_image_datatype(unsigned int xsize, unsigned int ysize)
{
  image_datatype image;

  /* check parameters */
  if( xsize == 0 || ysize == 0 ) error("new_image_datatype: invalid image size.");

  /* get memory */
  image = (image_datatype) malloc( sizeof(struct image_datatype_s) );
  if( image == NULL ) error("not enough memory.");
  image->data = (DataType *) calloc( (size_t) (xsize*ysize), sizeof(DataType) );
  if( image->data == NULL ) error("not enough memory.");
 ...
 */
static image_datatype new_image_datatype_ptr( unsigned int xsize,
                                          unsigned int ysize, DataType * data )
{
  image_datatype image;

  /* check parameters */
  if( xsize == 0 || ysize == 0 )
    error("new_image_datatype_ptr: invalid image size.");
  if( data == NULL ) error("new_image_datatype_ptr: NULL data pointer.");
...
/** Scale the input image 'in' by a factor 'scale' by Gaussian sub-sampling.
    ...
    in the y axis.
 */
static image_datatype gaussian_sampler( image_datatype in, double scale,
                                      double sigma_scale )
{
  image_datatype aux,out;
  ntuple_list kernel;
  ...
  aux = new_image_datatype(N,in->ysize);
  out = new_image_datatype(N,M);
      ...
      for(y=0;y<aux->ysize;y++)
        {
          ...
          aux->data[ x + y * aux->xsize ] = (DataType)sum;
        }
    }
      ...
      for(x=0;x<out->xsize;x++)
        {
          ...
          out->data[ x + y * out->xsize ] = (DataType)sum;
        }
    }

  /* free memory */
  free_ntuple_list(kernel);
  free_image_datatype(aux);
 ...
 */
static image_datatype ll_angle( const image_datatype in, double threshold,
                              struct coorlist ** list_p, void ** mem_p,
                              image_datatype * modgrad, unsigned int n_bins )
{
  image_datatype g;
  ...
  g = new_image_datatype(in->xsize,in->ysize);

  /* get memory for the image of gradient modulus */
  *modgrad = new_image_datatype(in->xsize,in->ysize);
    ...
    for(y=0;y<n-1;y++)
      {
        ...
        (*modgrad)->data[adr] = (DataType)norm; /* store gradient norm */
        ...
        if( norm <= threshold ) /* norm too small, gradient no defined */
        ...
        else
          {
            ...
            g->data[adr] = (DataType)atan2(gx,-gy);
 ...
 */
static int isaligned( int x, int y, image_datatype angles, double theta,
                      double prec )
{
  double a;
 ...
 */
static double rect_nfa(struct rect * rec, image_datatype angles, double logNT)
{
  rect_iter * i;
  int pts = 0;
  int alg = 0;
...
static double get_theta( struct point * reg, int reg_size, double x, double y,
                         image_datatype modgrad, double reg_angle, double prec )
{
...
static void region2rect( struct point * reg, int reg_size,
                         image_datatype modgrad, double reg_angle,
 ...
 */
static void region_grow( int x, int y, image_datatype angles, struct point * reg,
 ...
 */
static double rect_improve( struct rect * rec, image_datatype angles,
...
static int reduce_region_radius( struct point * reg, int * reg_size,
                                 image_datatype modgrad, double reg_angle,
                                 double prec, double p, struct rect * rec,
                                 image_char used, image_datatype angles,
                                 double density_th )
{
 ...
 */
static int refine( struct point * reg, int * reg_size, image_datatype modgrad,
                   double reg_angle, double prec, double p, struct rect * rec,
                   image_char used, image_datatype angles, double density_th )
{
...
double * LineSegmentDetection( int * n_out,
                               const DataType * img, int X, int Y,
                               ...
                               int ** reg_img, int * reg_x, int * reg_y )
{
  ...
  double * return_value;
  image_datatype scaled_image,angles,modgrad;
  image_char used;
  ...
  image = new_image_datatype_ptr( (unsigned int) X, (unsigned int) Y, (DataType*)img );
  if( scale != 1.0 )
    {
      scaled_image = gaussian_sampler( image, scale, sigma_scale );
      angles = ll_angle( scaled_image, rho, &list_p, &mem_p,
                         &modgrad, (unsigned int) n_bins );
      free_image_datatype(scaled_image);
    }
  else
  ...
  */
  ...
  free_image_datatype(angles);
  free_image_datatype(modgrad);
...
double * lsd_scale_region( int * n_out,
                           DataType * img, int X, int Y, double scale,
                           int ** reg_img, int * reg_x, int * reg_y )
{
  /* LSD parameters */
 ...
 */
double * lsd_scale(int * n_out, DataType * img, int X, int Y, double scale)
{
 ...
 */
double * lsd(int * n_out, DataType * img, int X, int Y)
{
  /* LSD parameters */
