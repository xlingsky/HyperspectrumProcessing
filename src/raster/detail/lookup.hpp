...
class LookupTable : public Lookup<_SrcType, _DstType>{
 ...
 public:
  ...
  DstType operator[](SrcType x) const override{
    int i;
    if(x<0) i=0;
    else if(x>_table.size()-1) i = _table.size()-1;
    else i = (int)x;
    return _table[i];
  }
  template<class Iterator>
...
class LookupMap : public LookupTable<_SrcType, _DstType>{
 public:
  typedef _SrcType SrcType;
  typedef _DstType DstType;
  typedef Range<double> Buckets;
  typedef LookupTable<_SrcType, _DstType> Base;
 protected:
  Buckets _buckets;
 public:
  LookupMap() {}
  void set_buckets(double start , double end , double step ){
    _buckets.reset(start, end, step);
  }
  DstType operator[](SrcType x) const override{
    return Base::operator[](_buckets.bucket(x));
  }
};
